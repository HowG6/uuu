...
random_spawn = True
# [CUT_IN_SCENARIO] Shared deterministic layout used by the UDMC entry point.
cut_in_layout = None
...
# init environment
# [MAP_LOADING_FIX] Match the stable optimized Town03 asset used by UDMC.
env = Env(map_id='03_Opt', display_method='spec', birdeye_view=False, dt=simu_step, recording=True)
...
# [CUT_IN_SCENARIO] Use the exact same fixed geometry and surrounding-vehicle
# behavior as udmc_main.py; only the ego controller differs in this baseline.
...
# [CUT_IN_SCENARIO] Match the initial ego state used in the UDMC experiment.
if scenario == 'cut_in':
...
if random_spawn:
    ...
    transform_list = []
    if scenario == 'cut_in':
        # [CUT_IN_SCENARIO] Roles/configs are shared with udmc_main.py.
...
if scenario == 'cut_in':
    # [CUT_IN_SCENARIO] Traffic Manager cannot guarantee an identical trigger
    # and trajectory, so the two surrounding vehicles use scripted agents.
...
for _ in range(10000):
    try:
        ...
        if scenario == 'cut_in':
            # [CUT_IN_SCENARIO] This baseline advances the world directly rather
            # than through Env.step(), so update manual surrounding agents here.
            env.update_other_agents()
        env.world.tick()
