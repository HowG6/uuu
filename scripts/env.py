...
#                                 PixelDimensions)
from others_agent import OthersAgent, FixedPointCutInAgent
...
class Env:
    def __init__(self, host="localhost", port=2000, map_id='05', birdeye_view=False, 
        ...
        # [MAP_LOADING_FIX] Town reloads can take longer than 10 seconds,
        # especially on the first low-quality/off-screen load. Allow an env var
        # override while using a practical default for CARLA 0.9.15.
        self.client.set_timeout(float(os.environ.get('CARLA_TIMEOUT', '60.0')))
        ...
        # [VIDEO_RECORDING] Initialized lazily after the RGB camera is spawned.
        self.camera_video_writer = None
        self.video_path = None
        # Keep cleanup safe even if scenario validation fails before reset().
        self.actor_list = []
        ...
        if not self.is_Leaderboard:
            ...
            # [MAP_LOADING_FIX] Compare the actual CARLA map asset name instead
            # of relying on the string representation of the Map object.
            current_map_name = self.map.name.split('/')[-1]
            if current_map_name == map_name:
                logging.info('Already loaded map {}, skipped'.format(map_name))
            else:
                self.world = self.client.load_world(map_name)
                # Obtain the server's current World handle after the reload;
                # the pre-load handle and cached objects are no longer valid.
                self.world = self.client.get_world()

            # [MAP_LOADING_FIX] load_world() returns a new World. Refresh the
            # cached map so waypoint-based scenarios never use the old map.
            self.map = self.world.get_map()
            logging.info('Current CARLA map: %s', self.map.name)
    ...
    def reset(self, ego_transform=None):
        ...
        if self.ego_vehicle is not None:
            ...
            if self.recording:
                ...
                # [VIDEO_RECORDING] Write the sensor stream directly to MP4.
                # This avoids thousands of PNG files and guarantees a playable
                # artifact when the simulation exits normally or by exception.
                video_dir = os.path.abspath('videos')
                os.makedirs(video_dir, exist_ok=True)
                video_name = self.dir_name.replace(':', '-') + '.mp4'
                self.video_path = os.path.join(video_dir, video_name)
                self.camera_video_writer = cv2.VideoWriter(
                    self.video_path,
                    cv2.VideoWriter_fourcc(*'mp4v'),
                    max(1, int(round(1.0 / DT_))),
                    (RES_X, RES_Y))
                if not self.camera_video_writer.isOpened():
                    raise RuntimeError('Unable to open video writer: {}'.format(self.video_path))
                self.camera.listen(self._record_camera_frame)
                print('[VIDEO_RECORDING] saving camera video to {}'.format(self.video_path))
    ...
    # [VIDEO_RECORDING] CARLA RGB raw_data uses BGRA byte order, while OpenCV
    # VideoWriter expects BGR.
    def _record_camera_frame(self, image):
    ...
    def spawn_other_vehicles(self, transform_list, auto_mode=False, agent_configs=None):
        bp_lib = self.world.get_blueprint_library()
        bp_types = []
        self.spawn_other_agents = []
        # [CUT_IN_SCENARIO] Stable role-to-actor lookup for scripted scenarios.
        self.other_vehicles_by_role = {}
        self.other_vehicles_auto = auto_mode
        if agent_configs is None:
            agent_configs = [{} for _ in transform_list]
        if len(agent_configs) != len(transform_list):
            raise ValueError('agent_configs and transform_list must have the same length')
        ...
        for vehicle_i in range(len(transform_list)):
            config = agent_configs[vehicle_i]
            ...
            if vehicle is not None:
                # [CUT_IN_SCENARIO] In synchronous mode a newly spawned actor
                # reports the default (0, 0) transform until the next tick.
                # Tick before constructing its waypoint-following agent.
                self.world.tick()
                # [CUT_IN_SCENARIO] Assign the requested initial velocity along
                # the lane heading. CARLA expects m/s while configs use km/h.
                initial_speed = float(config.get('initial_speed', 0.0)) / 3.6
                if initial_speed > 0.0:
                    forward = transform.get_forward_vector()
                    vehicle.set_target_velocity(carla.Vector3D(
                        x=initial_speed * forward.x,
                        y=initial_speed * forward.y,
                        z=initial_speed * forward.z))
                role = config.get('role', 'vehicle_{}'.format(vehicle_i))
                self.other_vehicles_by_role[role] = vehicle
                ...
                if auto_mode:
                    ...
                    # [CUT_IN_SCENARIO] Only rear_sv receives the deterministic
                    # fixed-position cut-in behavior; other traffic remains a
                    # normal lane-following OthersAgent.
                    if config.get('behavior') == 'fixed_point_cut_in':
                        agent = FixedPointCutInAgent(
                            vehicle,
                            trigger_waypoint=config['trigger_waypoint'],
                            overtake_speed=config.get('overtake_speed', 45.0),
                            settled_speed=config.get('settled_speed', 38.0),
                            lane_change_length=config.get('lane_change_length', 25.0))
                    else:
                        agent = OthersAgent(vehicle)
                        agent.set_target_speed(config.get('target_speed', 10.0))
                ...
                pass
        # tick the world
        # [CUT_IN_SCENARIO] Manual surrounding agents must be enabled so that
        # Env.step() actually calls update_other_agents().
        self.spawn_others_bool = len(self.other_vehicles_list) > 0
        self.world.tick()
    ...
    def step(self, state, transform_mode=False):
        ...
        if not transform_mode:
            ...
            for i in range(HFC_HZ):
                ...
                if not self.is_Leaderboard:
                    ...
                    # [CUT_IN_SCENARIO] Apply ego and surrounding-vehicle
                    # controls before the same synchronous tick. This keeps the
                    # scripted event timing identical across ego controllers.
                    if self.spawn_others_bool and not self.other_vehicles_auto:
        ...
        if transform_mode and self.spawn_others_bool and not self.other_vehicles_auto:
            self.update_other_agents()
    ...
    def clean(self):
        ...
        # [VIDEO_RECORDING] Stop callbacks before closing the MP4 container so
        # its index is finalized and the result is playable.
        if hasattr(self, 'camera') and self.camera is not None:
            try:
                self.camera.stop()
            except RuntimeError:
                pass
        if self.camera_video_writer is not None:
            self.camera_video_writer.release()
            self.camera_video_writer = None
            logging.info('video saved to %s', self.video_path)
