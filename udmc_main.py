...
from scripts.cut_in_scenario import build_cut_in_layout
...
random_spawn = False
# [CUT_IN_SCENARIO] Populated only for the deterministic fixed-point cut-in run.
cut_in_layout = None
# [VIDEO_RECORDING] Enable with UDMC_RECORD_VIDEO=True. The output path is
# printed at startup and files are stored in the workspace videos/ directory.
record_video = os.environ.get('UDMC_RECORD_VIDEO', 'False') == 'True'
...
# init environment
# [MAP_LOADING_FIX] CARLA 0.9.15 on this host crashes while hot-loading the
# legacy Town03 asset; Town03_Opt has the same road coordinates and loads
# reliably.
env = Env(map_id='03_Opt', display_method='spec', birdeye_view=False,
          dt=simu_step, recording=record_video)
...
if len(sys.argv) > 1:
    ...
    scenario = 'roundabout'
    random_spawn = 'False'
if scenario not in ['roundabout', 'multilaneACC', 'crossroad', 'comprehensive', 'unsig_crossroad', 'mixed_traffic', 'cut_in']:
...
# [CUT_IN_SCENARIO] Ego starts in the left lane near the Town03 intersection.
# The two right-lane vehicles and the fixed trigger are derived from map
# waypoints, so every controller/baseline sees the same physical event.
elif scenario == 'cut_in':
    if random_spawn:
        raise ValueError('cut_in is deterministic; run it with random_spawn=False')
    target_v = 30
    # Town03_Opt road 6: left lane remains non-junction from x~=165 to x~=220.
    ego_anchor = carla.Location(x=180.0, y=5.7, z=0.1)
    cut_in_layout = build_cut_in_layout(env.map, ego_anchor)
    start_transform = cut_in_layout['ego_transform']
    # Continue through the intersection so the route queue outlives the cut-in.
    destination_transform = carla.Transform(
        carla.Location(x=233.4, y=71.4, z=0.1),
        carla.Rotation(pitch=0.0, yaw=91.4, roll=0.0))
    traffic_lights = env.world.get_actors().filter('*traffic_light*')
    for traffic_light in traffic_lights:
        traffic_light.set_state(carla.TrafficLightState.Green)
        traffic_light.freeze(True)
    trigger = cut_in_layout['trigger_transform'].location
    print('[CUT_IN_SCENARIO] fixed trigger: x={:.2f}, y={:.2f}'.format(
        trigger.x, trigger.y))
elif scenario == 'crossroad':
    # all traffic lights are red and frozen
    traffic_lights = env.world.get_actors().filter('*traffic_light*')
    for traffic_light in traffic_lights:
    ...
    # all traffic lights are green and frozen
    traffic_lights = env.world.get_actors().filter('*traffic_light*')
    for traffic_light in traffic_lights:
        traffic_light.set_state(carla.TrafficLightState.Green)
        traffic_light.freeze(True)
...
# [CUT_IN_SCENARIO] Give ego a reproducible nonzero spawn velocity. The UDMC
# controller remains responsible for all subsequent ego motion.
if scenario == 'cut_in':
    ego_speed = cut_in_layout['config']['ego_initial_speed'] / 3.6
    ego_forward = start_transform.get_forward_vector()
    env.ego_vehicle.set_target_velocity(carla.Vector3D(
        x=ego_speed * ego_forward.x,
        y=ego_speed * ego_forward.y,
        z=ego_speed * ego_forward.z))
...
if random_spawn:
    ...
    transform_list = []
    # [CUT_IN_SCENARIO] Keep role-specific configs aligned with the two fixed
    # transforms: index 0 is front_sv and index 1 is rear_sv.
    if scenario == 'cut_in':
        transform_list = cut_in_layout['vehicle_transforms']
        agent_configs = cut_in_layout['agent_configs']
    elif scenario == 'mixed_traffic':  # Because mixed_traffic is a special scenario with custom positions
...
# other_vehicles spawn points and control mode (auto via Traffic Manager or manual)
# env.spawn_other_vehicles(transform_list, auto_mode=True) # Traffic Manager control mode is set to auto_mode=True, which means the other vehicles will be controlled by the Traffic Manager. If set to False, the other vehicles will be controlled manually.
if scenario == 'cut_in':
    # [CUT_IN_SCENARIO] Scripted agents are required for a fixed trigger and
    # deterministic smooth trajectory; Traffic Manager is intentionally off.
    env.spawn_other_vehicles(
        transform_list, auto_mode=False, agent_configs=agent_configs)
else:
    env.spawn_other_vehicles(transform_list, auto_mode=False) # user setting
...
# [CUT_IN_SCENARIO] A bounded step count makes repeatable experiments and short
# recording checks possible without changing the default 10000-step behavior.
max_steps = int(os.environ.get('UDMC_MAX_STEPS', '10000'))
for _ in range(max_steps):
    try:
        # [CUT_IN_SCENARIO] Xagent indexes queue elements 0..3. Finish cleanly
        # when the planned route has fewer than four points remaining.
        if scenario == 'cut_in' and len(agent._waypoints_queue) < 4:
            print('[CUT_IN_SCENARIO] ego route completed; stopping simulation')
            break
