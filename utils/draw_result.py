...
def plot_result(scenario='test', path=os.path.join(os.getcwd(), 'data', str(datetime.datetime.now()))):
    ...
    # [VIDEO_RECORDING] Create the parent data directory on a fresh checkout and
    # tolerate reruns that reuse the same output directory.
    os.makedirs(path, exist_ok=True)
    fig2.savefig(os.path.join(path, scenario+'_input.pdf'),
                 bbox_inches='tight', pad_inches=0)
    fig3.savefig(os.path.join(path, scenario+'_apf.pdf'),
                 bbox_inches='tight', pad_inches=0)
    # [VIDEO_RECORDING] Headless CARLA runs must not block forever waiting for
    # an interactive Matplotlib window. Opt in when a plot window is desired.
    if os.environ.get('UDMC_SHOW_PLOTS', 'False') == 'True':
        plt.show()
    else:
        plt.close('all')
